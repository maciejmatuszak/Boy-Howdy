...
auto main() -> int {
  ...
  ok &= expect(identify_calls == 1,
               "unknown exception path still calls identify once");
  identify_throw_mode = 0;
  ...
  ok &= expect(pam_sm_acct_mgmt(nullptr, 0, 0, nullptr) == PAM_IGNORE,
               "acct_mgmt ignores auth");
  ok &= expect(pam_sm_close_session(nullptr, 0, 0, nullptr) == PAM_IGNORE,
               "close_session ignores auth");
  ok &= expect(pam_sm_chauthtok(nullptr, 0, 0, nullptr) == PAM_IGNORE,
               "chauthtok ignores auth");
  ok &= expect(pam_sm_setcred(nullptr, 0, 0, nullptr) == PAM_IGNORE,
               "setcred ignores auth");
  ok &= expect(identify_calls == 0,
               "non-auth PAM entrypoints do not call identify");
