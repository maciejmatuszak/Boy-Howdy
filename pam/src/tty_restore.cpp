...
auto TtyRestoreContext::operator=(TtyRestoreContext &&other) noexcept -> TtyRestoreContext & {
    if (this == &other) {
        return *this;
    }
...
auto TtyRestoreContext::restore_echo(std::string *error_message) const -> bool {
    ...
    if (tcgetattr(fd_, &current) != 0) {
        if (error_message != nullptr) {
            *error_message = "Failed to read terminal state: " + std::string(std::strerror(errno));
        }
        return false;
    }
    ...
    if (tcsetattr(fd_, TCSANOW, &original_) != 0) {
        if (error_message != nullptr) {
            *error_message =
                "Failed to restore terminal echo: " + std::string(std::strerror(errno));
        }
        return false;
    }
...
auto TtyRestoreContext::write_newline(std::string *error_message) const -> bool {
    ...
    if (error_message != nullptr) {
        if (bytes_written < 0) {
            *error_message =
                "Failed to write terminal newline: " + std::string(std::strerror(errno));
        } else {
