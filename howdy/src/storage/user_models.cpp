...
namespace howdy::native {
    ...
    auto load_user_models(const std::string &user, const std::string &expected_backend,
        ...
        const auto models_dir_security = check_secure_root_owned_directory_tree(
            user_models_dir, "User models directory", owner_uid);
        ...
        if (size_ec || file_size > kMaxUserModelFileBytes) {
            ...
            result.error_message =
                "User model file is too large or unreadable: " + model_path->string();
            return result;
        }
        ...
        try {
            for (const auto &model : models) {
                ...
                if (data->size() > kMaxEncodingsPerModel) {
                    result.status        = UserModelStatus::kParseError;
                    result.error_message = "Stored face model contains too many encodings";
                    return result;
                }
                ...
                for (const auto &encoding_json : *data) {
                    ...
                    if (encoding_json.empty() || encoding_json.size() > kMaxEncodingLength) {
                        result.status        = UserModelStatus::kParseError;
                        result.error_message = "Stored face encoding exceeds safety limit";
                        return result;
                    }
