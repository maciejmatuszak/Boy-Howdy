...
namespace {
	...
	auto CapabilitiesEmpty(const CapabilityData &data, bool check_inheritable) -> bool {
	...
	auto ClearAndVerifyCapabilities(
...
namespace howdy::native::compare_privileges_internal {
	...
	auto FatalVerificationFailure(const ComparePrivilegeDependencies &dependencies)
	...
	[[noreturn]] void FatalComparePrivilegeFailure([[maybe_unused]] void *context,
	...
	auto ReadProcessIdentity(const ComparePrivilegeDependencies &dependencies,
	...
	auto HandleUnprivilegedCaller(const ComparePrivilegeDependencies &dependencies, uid_t real_uid,
	...
	auto PrepareCapabilityDrop(const ComparePrivilegeDependencies &dependencies)
	    -> ComparePrivilegeResult {
	...
	auto TransitionIdentity(const ComparePrivilegeDependencies &dependencies, uid_t target_uid,
	...
	auto VerifyDroppedIdentity(const ComparePrivilegeDependencies &dependencies, uid_t target_uid,
	                           gid_t target_gid) -> bool {
		__user_cap_header_struct capability_header{
		    .version = _LINUX_CAPABILITY_VERSION_3,
		    .pid     = 0,
		};
		if (!ClearAndVerifyCapabilities(dependencies, capability_header)) {
			return false;
		}
