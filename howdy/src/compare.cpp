  ...
  try {
    ...
    while (true) {
      ...
      if (!has_sane_frame_dimensions(gray_frame)) {
        std::cerr << "Camera returned invalid frame dimensions: "
                  << gray_frame.cols << "x" << gray_frame.rows << "\n";
        return static_cast<int>(CompareExit::kInvalidDevice);
      }
      ...
      const auto darkness = hist_total == 0.0
                                ? 100.0F
                                : static_cast<float>(hist.at<float>(0) / hist_total *
                                                     100.0);
      ...
      for (const auto &face : faces) {
        ...
        const auto match =
            face_model.best_match(loaded_models.stored.encodings, encoding);
        ...
        if (end_report) {
          ...
          if (winning_index >= 0 &&
              winning_index <
                  static_cast<int>(loaded_models.stored.models.size())) {
            const auto &winner =
