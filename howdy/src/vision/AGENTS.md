...
- `video_capture.cpp` mirrors OpenCV `VideoCapture` open/read/set/get behavior.
- Callers must handle camera open/read failures explicitly.
- Validate captured `cv::Mat` through `vision/frame_validation.hpp` before
