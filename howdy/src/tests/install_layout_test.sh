...
direct_howdy_src_stage="$build_dir/direct howdy-src install root-ทดสอบ"
...
direct_howdy_src_log="$build_dir/direct-howdy-src-install-prefix.log"
...
cleanup() {
	rm -rf "$stage" "$same_prefix_stage" "$override_stage" "$override_log" \
		...
		"$direct_pam_po_stage" "$direct_howdy_src_stage" "$direct_pam_log" \
		"${direct_pam_log}.out" "$direct_howdy_log" "${direct_howdy_log}.out" \
		"$direct_pam_po_log" "${direct_pam_po_log}.out" "$direct_howdy_src_log" \
		"${direct_howdy_src_log}.out" "$literal_prefix_root"
}
...
reject_direct_prefix "$build_dir/howdy/src" "$direct_howdy_src_stage" "$direct_howdy_src_log"
