...
{"id":8,"label":"second","backend":"opencv_dnn_sface","data":["ignored",[1.1,1.2,1.3]]}
	...
	};
	{
		...
		const howdy::native::user_model_store_test_hooks::ScopedHooks hooks({
		    .after_lock_before_revalidate =
		        [&hook](const std::filesystem::path &path) {
			        replace_with_victim_symlink_after_lock(&hook, path);
		        },
		});
		const howdy::native::UserModelStore                           store;
		const auto result = store.begin_mutation("alice");
		...
		ok &= expect(result.document.result.status == howdy::native::UserModelStatus::kInsecurePath,
		...
		ok &= expect(read_file(victim_path) == victim_content,
	...
	}
	{
		...
		const howdy::native::UserModelStore                           store;
		const auto result = store.lock_existing("alice");
		ok &= expect(hook.calls == 1,
	...
	}
	{
		...
		const auto result = store.begin_mutation("alice");
		ok &=
		    expect(hook.calls == 1, "begin_mutation regular-file swap hook executes exactly once");
		ok &=
		    expect(hook.swapped, "begin_mutation hook atomically replaces model with regular file");
		ok &= expect(result.document.result.status == howdy::native::UserModelStatus::kModelChanged,
	...
	}
	{
		...
		const howdy::native::UserModelStore                           store;
		const auto result = store.lock_existing("alice");
		ok &= expect(hook.calls == 1, "lock_existing regular-file swap hook executes exactly once");
		ok &=
		    expect(hook.swapped, "lock_existing hook atomically replaces model with regular file");
		ok &= expect(result.status == howdy::native::UserModelStatus::kModelChanged,
	...
	}
	{
		...
		for (const auto &entry : fs::directory_iterator(models_dir)) {
			found_write_temp |= entry.path().filename().string().starts_with(".howdy-user-model-");
		}
		ok &= expect(found_write_temp, "write cleanup failure leaves injected temporary artifact");
		const auto cleanup_retry = howdy::native::append_user_model_entry("alice", second_entry);
	...
	ec.clear();
	if (symlink("/tmp", stale_lock_path.c_str()) == 0) {
		const auto result = howdy::native::append_user_model_entry("alice", first_entry);
		ok &= expect(result.status == howdy::native::UserModelStatus::kOk,
		...
		std::cerr << "SKIP: stale lock symlink creation failed\n";
	}
