...
namespace {
	...
	auto test_global_options() -> bool {
		...
		for (std::size_t index = 0; index < options.size(); ++index) {
			...
			const auto &[id, short_name, long_name, argument_name] = expected_options[index];
			ok &= expect(option.id == id && option.short_name == short_name &&
			                 option.long_name == long_name && option.argument_name == argument_name,
			             "global option syntax and order are stable");
