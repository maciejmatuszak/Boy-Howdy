...
namespace howdy::test::config_cli {
	...
	namespace {
		...
		auto read_snapshot(void *raw, const std::filesystem::path &path, std::string *content)
		    -> bool {
			auto &context = *static_cast<TestContext *>(raw);
			record(context, 6);
	...
	auto run_config_cli_callback_exception_test() -> bool {
		return stdout_restores_after_callback_exception();
	}
	...
	auto run_config_cli_workflow_tests() -> bool {
...

		{
			...
			ok &= expect_installer_arguments(context,
