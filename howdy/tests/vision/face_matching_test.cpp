...
auto main() -> int {
	...
	{
		const auto match = howdy::native::FindBestFaceMatch({{1.0F, 2.0F}}, {1.0F, 2.0F},
		                                                    FaceMetric::kCosine, 0.9F);
		ok &= Expect(match.index == 0, "cosine exact positive match has valid index");
		ok &= ExpectNear(match.score, 1.0F, 1.0e-6F, "cosine exact positive match scores one");
		ok &= Expect(match.accepted, "cosine exact positive match is accepted");
	}

	{
		const auto match = howdy::native::FindBestFaceMatch({{0.0F, 1.0F}}, {1.0F, 0.0F},
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{-1.0F}}, {1.0F}, FaceMetric::kCosine, -1.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{-1.0F}}, {1.0F}, FaceMetric::kCosine, -0.9F);
		ok &= Expect(!match.accepted, "antiparallel cosine match below threshold is rejected");
	}

	{
		const auto nan   = std::numeric_limits<float>::quiet_NaN();
		const auto match = howdy::native::FindBestFaceMatch(
...

	{
		...
		const auto match    = howdy::native::FindBestFaceMatch({{1.0F, 0.0F}}, {infinity, 0.0F},
	...
	{
		const auto match = howdy::native::FindBestFaceMatch({{1.0F, 0.0F}}, {0.0F, 0.0F},
		                                                    FaceMetric::kCosine, -1.0F);
	...
	{
		const auto match = howdy::native::FindBestFaceMatch({{0.0F, 0.0F}}, {1.0F, 0.0F},
		                                                    FaceMetric::kCosine, 0.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{1.0F, 2.0F}}, {1.0F, 2.0F}, FaceMetric::kL2, 0.5F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{3.0F, 4.0F}}, {0.0F, 0.0F}, FaceMetric::kL2, 5.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{3.0F, 4.0F}}, {0.0F, 0.0F}, FaceMetric::kL2, 4.9F);
...

	{
		...
		const std::vector<float> non_finite_values = {std::numeric_limits<float>::quiet_NaN(),
		                                              infinity, -infinity};
		for (const float non_finite : non_finite_values) {
			const auto match = howdy::native::FindBestFaceMatch({{1.0F, 2.0F}}, {non_finite, 2.0F},
			                                                    FaceMetric::kL2, 0.5F);
	...
	{
		const auto match = howdy::native::FindBestFaceMatch({{0.0F, 2.0F}}, {0.0F, 0.0F},
...

	{
		...
		ok &= ExpectNear(match.score, std::numbers::sqrt2_v<float>, 1.0e-6F,
		                 "best candidate has lowest distance");
		ok &= Expect(match.accepted, "best candidate within threshold is accepted");
	}

	{
		const auto match = howdy::native::FindBestFaceMatch({{0.0F}, {1.0F, 1.0F}}, {1.0F, 1.0F},
		                                                    FaceMetric::kL2, 0.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{1.0F}}, {1.0F, 2.0F}, FaceMetric::kCosine, 0.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{1.0F}}, {1.0F, 2.0F}, FaceMetric::kL2, 1.0F);
	...
	{
		const auto match = howdy::native::FindBestFaceMatch({}, {1.0F}, FaceMetric::kCosine, 0.0F);
	...
	{
		const auto match =
		    howdy::native::FindBestFaceMatch({{1.0F}}, {}, FaceMetric::kL2Norm, 1.0F);
	...
	ok &= Expect(!howdy::native::ParseFaceMetric("euclidean").has_value(),
	             "unknown face metric is rejected by typed parser");
	{
		// NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
		const auto invalid_metric = static_cast<FaceMetric>(255);
		const auto match = howdy::native::FindBestFaceMatch({{1.0F}}, {1.0F}, invalid_metric, 0.0F);
...

	{
		...
		ok &= Expect(howdy::native::FaceMetricThresholdMaximum() == expected_max,
