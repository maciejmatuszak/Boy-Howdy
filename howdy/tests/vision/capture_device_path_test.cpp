...
auto main() -> int {
	...
	ok &= expect(howdy::native::is_allowed_capture_device_path("/dev/v4l/by-id/missing-camera"),
	...
	ok &= expect(!howdy::native::is_allowed_capture_device_path("/dev/v4l/by-id/../../null"),
	...
	ok &= expect(!howdy::native::is_allowed_capture_device_path("/dev/v4l/by-id/.."),
	...
	ok &= expect(
	    !howdy::native::detail::is_allowed_capture_device_path(fake_regular.string(), temp_root),
	    "regular file in direct video namespace is rejected");

	const auto missing_by_id = temp_root / "dev" / "v4l" / "by-id" / "truly-missing-camera";
	ok &= expect(
	    howdy::native::detail::is_allowed_capture_device_path(missing_by_id.string(), temp_root),
	...
	ok &= expect(
	    howdy::native::detail::is_allowed_capture_device_path(missing_by_path.string(), temp_root),
	...
	ok &= expect(
	    !howdy::native::detail::is_allowed_capture_device_path(dangling_by_id.string(), temp_root),
	...
	ok &= expect(!howdy::native::detail::is_allowed_capture_device_path(dangling_by_path.string(),
	                                                                    temp_root),
	             "dangling by-path symlink is rejected");

	const auto fake_video0 = temp_root / "dev" / "video0";
	ok &= expect(write_file(fake_video0, "fake video device content"),
	...
	ok &= expect(!howdy::native::detail::is_allowed_capture_device_path(
	                 regular_target_by_id.string(), temp_root),
	...
	ok &= expect(!howdy::native::detail::is_allowed_capture_device_path(
	                 outside_target_by_id.string(), temp_root),
