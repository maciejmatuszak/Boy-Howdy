...
auto main() -> int {
	...
	auto disabled_clahe = howdy::native::MakeClahe(disabled_config);
	...
	howdy::native::ApplyClaheIfEnabled(disabled_frame, disabled_config, disabled_clahe);
	...
	auto enabled_clahe = howdy::native::MakeClahe(enabled_config);
	...
	howdy::native::ApplyClaheIfEnabled(enabled_frame, enabled_config, enabled_clahe);
	...
	howdy::native::ApplyClaheIfEnabled(empty_enabled_frame, enabled_config, empty_enabled_clahe);
	...
	ok &= Expect(
	    howdy::native::ValidateFrame(cv::Mat(), howdy::native::FrameChannelPolicy::kCameraInput) ==
	...
	ok &= Expect(
	    howdy::native::ValidateFrame(three_d_frame, howdy::native::FrameChannelPolicy::kGray) ==
	        howdy::native::FrameValidationStatus::kUnsupportedDimensions,
	    "non-empty 3-D frame validation rejects unsupported dimensions");
	ok &= Expect(howdy::native::ValidateFrame(cv::Mat(1, 1, CV_32FC1, cv::Scalar(0.0F)),
	...
	ok &= Expect(howdy::native::ValidateFrame(max_boundary_frame,
	                                          howdy::native::FrameChannelPolicy::kGray) ==
	                 howdy::native::FrameValidationStatus::kValid,
	             "8192x1 grayscale frame is valid");
	ok &= Expect(
	    howdy::native::ValidateFrame(cv::Mat(howdy::native::kMaxFrameDimension + 1, 1, CV_8UC1),
	                                 howdy::native::FrameChannelPolicy::kGray) ==
	        howdy::native::FrameValidationStatus::kOversizedDimensions,
	    "8193x1 grayscale frame is oversized");
	ok &= Expect(
	    howdy::native::ValidateFrame(cv::Mat(1, howdy::native::kMaxFrameDimension + 1, CV_8UC1),
	                                 howdy::native::FrameChannelPolicy::kGray) ==
	        howdy::native::FrameValidationStatus::kOversizedDimensions,
	    "1x8193 grayscale frame is oversized");
	ok &= Expect(howdy::native::ValidateFrame(cv::Mat(1, 1, CV_8UC1),
	                                          howdy::native::FrameChannelPolicy::kCameraInput) ==
	                 howdy::native::FrameValidationStatus::kValid,
	             "raw validation accepts 1 channel");
	ok &= Expect(howdy::native::ValidateFrame(cv::Mat(1, 1, CV_8UC3),
	                                          howdy::native::FrameChannelPolicy::kCameraInput) ==
	                 howdy::native::FrameValidationStatus::kValid,
	             "raw validation accepts 3 channels");
	ok &= Expect(howdy::native::ValidateFrame(cv::Mat(1, 1, CV_8UC4),
	                                          howdy::native::FrameChannelPolicy::kCameraInput) ==
	                 howdy::native::FrameValidationStatus::kValid,
	             "raw validation accepts 4 channels");
	ok &= Expect(howdy::native::ValidateFrame(cv::Mat(1, 1, CV_8UC2),
	                                          howdy::native::FrameChannelPolicy::kCameraInput) ==
	...
	const auto empty_brightness = howdy::native::MeasureBrightness(cv::Mat());
	...
	const auto    black_brightness = howdy::native::MeasureBrightness(black_frame);
	...
	if (black_brightness.hist_total == 0.0 || black_brightness.darkness >= 100.0F) {
		...
		switch (howdy::native::ClassifyBrightness(black_brightness.hist_total,
	...
	ok &= Expect(howdy::native::ClassifyBrightness(black_brightness.hist_total,
	                                               black_brightness.darkness, 30.0F) ==
	...
	const auto normal_brightness = howdy::native::MeasureBrightness(normal_frame);
	ok &= ExpectNear(normal_brightness.darkness, 25.0, 0.0001,
	...
	for (std::size_t index = 0; index < expected_bins.size(); ++index) {
		ok &= ExpectNear(normal_brightness.bins_percent[index], expected_bins[index], 0.0001,
		                 "normal frame histogram bin percentage is stable");
	}
	ok &= Expect(howdy::native::ClassifyBrightness(normal_brightness.hist_total,
