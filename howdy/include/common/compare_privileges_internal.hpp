...
namespace howdy::native::compare_privileges_internal {
	...
	using GetresgidFn  = int (*)(void *context, gid_t *real, gid_t *effective, gid_t *saved);
	using GetresuidFn  = int (*)(void *context, uid_t *real, uid_t *effective, uid_t *saved);
	using QueryFsuidFn = uid_t (*)(void *context);
	using QueryFsgidFn = gid_t (*)(void *context);
	using SetuidFn     = int (*)(void *context, uid_t uid);
	using FatalExitFn  = void (*)(void *context, const char *message, std::size_t message_size,
	                              int exit_code);
	...
	[[noreturn]] void fatal_compare_privilege_failure(void *context, const char *message,
	                                                  std::size_t message_size, int exit_code);
