...
inline auto bounded_int_or_fallback(int value, int fallback, int minimum,
                                    int maximum) -> int {
  return value >= minimum && value <= maximum ? value : fallback;
}

inline auto bounded_float_or_fallback(float value, float fallback, float minimum,
                                      float maximum) -> float {
  return value >= minimum && value <= maximum ? value : fallback;
}
...
inline auto config_dark_threshold(const ConfigReader &config,
                                  float fallback = 60.0F) -> float {
  return bounded_float_or_fallback(
      config.get_float("video", "dark_threshold", fallback), fallback, 0.0F, 99.9F);
}

inline auto config_max_height(const ConfigReader &config) -> float {
  return bounded_float_or_fallback(config.get_float("video", "max_height", 320.0F),
                                   320.0F, 32.0F, 4096.0F);
}
...
inline auto config_clahe_clip_limit(const ConfigReader &config) -> float {
  return bounded_float_or_fallback(
      config.get_float("video", "clahe_clip_limit", 1.25F), 1.25F, 0.01F, 100.0F);
}

inline auto config_clahe_tile_grid_size(const ConfigReader &config) -> int {
  return bounded_int_or_fallback(config.get_int("video", "clahe_tile_grid_size", 8),
                                 8, 1, 64);
}
...
inline auto config_yunet_score_threshold(const ConfigReader &config) -> float {
  return bounded_float_or_fallback(
      config.get_float("face", "yunet_score_threshold", 0.9F), 0.9F, 0.0F, 1.0F);
}

inline auto config_yunet_nms_threshold(const ConfigReader &config) -> float {
  return bounded_float_or_fallback(
      config.get_float("face", "yunet_nms_threshold", 0.3F), 0.3F, 0.0F, 1.0F);
}

inline auto config_yunet_top_k(const ConfigReader &config) -> int {
  return bounded_int_or_fallback(config.get_int("face", "yunet_top_k", 5000), 5000,
                                 1, 10000);
}
...
inline auto config_sface_threshold(const ConfigReader &config,
                                   const std::string &metric) -> float {
