...
inline auto normalized_lower(std::string value) -> std::string {
  for (char &ch : value) {
...
inline auto parse_int_strict(std::string_view value) -> std::optional<int> {
  if (value.empty()) {
    return std::nullopt;
  }
...
inline auto parse_float_strict(std::string_view value) -> std::optional<float> {
  if (value.empty()) {
    return std::nullopt;
  }
...
inline auto is_valid_bool_text(std::string_view value) -> bool {
  const auto lowered = normalized_lower(std::string(value));
  return lowered == "true" || lowered == "false" || lowered == "1" ||
         lowered == "0" || lowered == "yes" || lowered == "no" ||
         lowered == "on" || lowered == "off";
}

inline auto invalid_config_value_message(const std::string_view key,
...
inline auto validate_known_config_value(const ConfigReader &config,
                                        std::string_view key,
                                        std::string_view value)
    -> std::optional<std::string> {
  if (key == "detection_notice" || key == "no_confirmation" ||
  ...
  if (key == "sface_metric") {
    ...
    if (lowered != "cosine" && lowered != "l2" && lowered != "l2norm") {
      return invalid_config_value_message(key, "expected one of: cosine, l2, l2norm");
    }
    return std::nullopt;
  }
  ...
  if (key == "frame_width" || key == "frame_height") {
    ...
    if (!parsed.has_value() || (*parsed != -1 && (*parsed < 16 || *parsed > 8192))) {
      return invalid_config_value_message(key, "expected -1 or integer range 16..8192");
    }
    return std::nullopt;
  }
  ...
  if (key == "device_fps") {
    const auto parsed = parse_int_strict(value);
    if (!parsed.has_value() || (*parsed != -1 && (*parsed < 0 || *parsed > 480))) {
      return invalid_config_value_message(key, "expected -1 or integer range 0..480");
    }
    return std::nullopt;
  }
  ...
  if (key == "exposure") {
    ...
    if (!parsed.has_value() || (*parsed != -1 && (*parsed < 0 || *parsed > 10000))) {
      return invalid_config_value_message(key, "expected -1 or integer range 0..10000");
    }
    return std::nullopt;
  }
  ...
  if (key == "sface_threshold") {
    const auto parsed = parse_float_strict(value);
    if (!parsed.has_value()) {
      return invalid_config_value_message(key, "expected a floating-point value");
    }
    const auto metric =
        normalized_lower(config.get("face", "sface_metric", "cosine"));
    const float maximum = metric == "cosine" ? 1.0F : 4.0F;
    if (*parsed < 0.0F || *parsed > maximum) {
      return invalid_config_value_message(
          key, metric == "cosine" ? "expected range 0..1" : "expected range 0..4");
    }
    return std::nullopt;
  }
...
inline auto validate_runtime_config(const ConfigReader &config)
    -> std::optional<std::string> {
